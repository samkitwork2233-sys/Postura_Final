...
function updateThresholdUI(){
  const slider = $("goodPostureSlider");
  if(slider) goodPostureAngle = Number(slider.value);

  const value = $("goodPostureValue");
  if(value) value.textContent = `${goodPostureAngle}°`;

  const readinessThreshold = $("readinessThreshold");
  if(readinessThreshold) readinessThreshold.textContent = `${goodPostureAngle}°`;

  const goodAngle = $("goodPostureAngle");
  if(goodAngle) goodAngle.textContent = `${goodPostureAngle}°`;
}
...
function updateLiveData(current, deviation){
  ...
  $("readinessAngle").textContent = `${currentDeviation.toFixed(1)}° deviation`;
...
async function runReadinessCheck(isRecheck=false){
  ...
  $("readinessAngle").textContent = `${angle.toFixed(1)}° deviation`;
  $("readinessThreshold").textContent = `${threshold}°`;
